...
from toolbox.file_tools import read_inputs, get_output_file_name, \
    write_config_string
from toolbox.flat_models import MTrumorFlatRadvarRandomWalk, \
    mtrumor_flat_radvar_random_walk_get_parameters
import numpy as np
import sys
import datetime
...
def simulate_n_times(model, input_dict):
    ...
    Parameters
    ----------
    model : MTrumorFlatRadvarRandomWalk
        Model
    input_dict : dict
        Input dictionary.
    """
    ...
    for n in range(num_simulations):
        ...
        # Redraws the random radii of interaction, to avoid biasing the final result.
        model.regenerate_all_deltas()
...
def write_results_to_file(out_file_name, i_2d_array,
                          delimiter="\t"):
...
def main():
    ...
    # Creates an MTrumor-model instance.
    model = MTrumorFlatRadvarRandomWalk(
        *mtrumor_flat_radvar_random_walk_get_parameters(input_dict)
    )
    ...
    write_results_to_file(out_file_name, i_2d_array)
